...
export function IconDialogBox() {
  ...
  return (
    <Dialog open={openDialog} onOpenChange={(open) => updateOpenDialog(open)}>
      ...
      <DialogContent className="max-w-2xl items-center max-sm:h-full">
        <DialogHeader>
        ...
        <div className="flex h-fit flex-wrap gap-2 rounded-lg border p-3 text-xl max-sm:justify-between">
          {allIcons.map((singleIcon, index) => (
            <div
              className={`h-fit w-fit rounded-md border p-3 hover:bg-primary hover:text-white ${
